...
# Disable SSL warnings (GitHub Enterprise)
...
def codeowners_exists(api, org, repo, branch="master"):
    ...
    for url in locations:
        ...
        if result:
            found_location = url
            print(f"DEBUG: CODEOWNERS found at {url} (API response: {result})")
            return True
        else:
            print(f"DEBUG: CODEOWNERS not found at {url} (API response: {result})")
    return False

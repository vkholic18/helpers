...
class CMDBClient:
    ...
    def remove_hosts_from_cmdb(
        ...
        # ✅ BUILD DEBUG INFO TO RETURN
        debug_info = {
            "url": self.CMDB_INSERT_MULTIPLE_PROD_API_URL,
            "payload": payload,
            "token": self.CMDB_ACCESS_TOKEN,
            "headers": headers,
        }
        ...
        try:
            ...
            response_json["_debug"] = debug_info

            print("Graveyarded the above hosts in the CMDB inventory successfully!!")
            return response_json  # ✅ FIXED - Now returns response

        except requests.exceptions.Timeout:
            # ✅ RETURN DEBUG INFO EVEN ON ERROR
            raise TimeoutError(
                f"CMDB timeout. Debug info: {json.dumps(debug_info, indent=2)}"
            )
            ...
            raise RuntimeError(f"CMDB error: {json.dumps(error_response, indent=2)}")
